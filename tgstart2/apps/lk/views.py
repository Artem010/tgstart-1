...
def activatebot(request):
    ...
    pr = subprocess.Popen(['python3', cDir +'/main.py']) #starting subprocess with bot
...
def mybots(request):
    ...
    if request.method == "POST":
        ...
        if not (mainUser.bot_set.filter(token=request.POST.get('tgToken')).exists()): #if there is no bot with the current token
            ...
            pr = subprocess.Popen(['python3', cDir + mainUserId + "/" + mainBotId +'/main.py']) #starting subprocess with bot's script
